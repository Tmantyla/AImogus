...
var GEMINI_CHAT_URL = "https://aiplatform.googleapis.com/v1/publishers/google/models/gemini-2.5-flash-lite:generateContent?key=%s" % GOOGLE_API_KEY
...
func transform_history_for_gemini(history):
	...
	for message in history:
		var transformed = {
			"role": message.role, # Typically "user" or "model"
			"parts": [
...
func send(messages: Array) -> String:
